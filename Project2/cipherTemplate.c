...
unsigned long readPositiveLong(char delimiter)
{
  ...
  //while (c!=delimiter && c!='\n' && c!=EOF)
  while (c>= '0' && c <= '9') //still reading when reach line 28
  { 
    num = (c - '0')+ temp*10;
    temp=num;
    c = getchar();
...
//byte 4 characters is one block
//special character reverse 
//4 char in array [B][y][T][E]
//given bit index 11. Then find the byte in the data block by 11/7 = 1, 
//masking to get test if the set to zero or one 11%4 = 4 wheere bit in the byte.
//f(i) at the index to equal to the bit in the data block.
//data is only 4 long for loop i<4;
//if there's a '*' then ignore that first, get the second one read only 4 chars at a time then the rest.
...
int readKey(void)
{ 
  lcg_m = readPositiveLong(',');
  lcg_c = readPositiveLong(',');
  lcg_a = calculateLCG_a(lcg_m);

  lcg_x = lcg_c;
  if(0 < lcg_m && lcg_a < lcg_m && lcg_c < lcg_m) return OK;
  return ERROR;
}
...
//need to do  checking for non printable ascii char.
...
{ 
  ...
  for(i = 0; i<4; i++) cipherText[i] = '\0';
  //for through Data bits and byte
  for(i = 0; i<MAP_LENGTH; i++)
  {
    k = cipherMap[i];
    byte = i/7;
  ...
  for(i = 0; i<4; i++)
  {
    if(cipherText[i] <32 && cipherText[i]!='\0') 
    {
      printf("*%c",'?'+cipherText[i]);
    }
    else if (cipherText[i]==127)
    {
      printf("*");
      putchar('%');
    } 
...
{ 
  ...
  { 
    ...
    // if(plainText[i]=='\0') return ERROR;
  }
  for(i = 0; i<4; i++)
  { 
    if (plainText[i]<' '|| plainText[i]>'~') return ERROR; 
    if(plainText[i] <32) 
    {
